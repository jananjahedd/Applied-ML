...
def display_model_performance(performance_data: Dict[str, Any]) -> None:
    ...
    if performance_data.get("performance_summary"):
        summary_df = pd.DataFrame.from_dict(performance_data["performance_summary"], orient="index")
...
with st.spinner("Fetching available model configurations..."):
    try:
        ...
        available_configs = [
            cfg
            for cfg, details in available_configs_data.get("available_configurations", {}).items()
            if details["available"]  # Only show truly available models
        ]
        default_config = available_configs_data.get(
            "default_configuration", available_configs[0] if available_configs else None
        )

        if not available_configs:
            st.error(
                "No trained models found. Please ensure models are in the 'results/' "
                "directory and the API is running correctly."
            )
            st.stop()
    ...
    with st.spinner(f"Loading details for '{selected_model_config}' model..."):
        ...
        if model_details:
            ...
            if performance_summary:
                ...
                st.info(
                    f"**Test Accuracy:** {performance_summary.get('test_accuracy', 'N/A'):.4f} | "
                    f"**Test Macro F1:** {performance_summary.get('test_macro_f1_score', 'N/A'):.4f}"
                )
            else:
                st.info(
                    "No quick performance summary available for this model. "
                    "Click 'Get Detailed Model Performance' below for full metrics."
                )


st.header("🚀 Model Performance Analysis")
st.markdown(
    "Dive deep into the performance of the selected model configuration, "
    "including training, validation, and test metrics."
)
...
st.markdown(
    "Select an EDF recording from the available files and run the sleep stage "
    "prediction pipeline using the chosen model configuration."
)
...
if st.button("Run Prediction"):
    if not selected_recording_id:
        ...
        prediction_endpoint = PREDICT_FROM_RECORDING_ENDPOINT.format(
            model_id=selected_model_config, recording_id=selected_recording_id
        )
        st.info(
            f"Running prediction for Recording ID `{selected_recording_id}` with model `{selected_model_config}`..."
        )
        with st.spinner("Processing recording and predicting sleep stages... This may take a moment."):
            try:
                predict_response = requests.post(
                    prediction_endpoint, timeout=300
                )  # Increased timeout for long processing
                ...
                st.write(
                    ...
                    f"{prediction_results.get('processing_summary', {}).get('total_recording_time_hours', 0):.2f} hours"
                )
                ...
                current_file_performance = prediction_results.get("processing_summary", {}).get(
                    "current_file_performance"
                )
