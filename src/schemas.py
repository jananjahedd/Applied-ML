...
class AvailableFilesResponse(BaseModel):
    cassette_files: Dict[str, str] = Field(..., example={"1": "patient1-PSG.edf"})
    telemetry_files: Dict[str, str] = Field(..., example={"3": "patient3-PSG.edf"})
...
class SelectedFilesResponse(BaseModel):
    selected_files: Dict[str, str] = Field(..., example={"1": "example-data/sleep-cassette/SCA121B-PSG.edf"})
    total_selected: int
...
class PreprocessingOutput(BaseModel):
    ...
    status: str
    message: str
    extracted_features: Optional[List[float]] = Field(None, example=[0.1, 0.5, 0.2, 1.3, 0.8])
    feature_names: Optional[List[str]] = Field(None, example=["EEG_Fpz_Cz_delta_RelP", "EEG_Fpz_Cz_theta_RelP", "EOG_horizontal_Var"])
...
class PredictionInput(BaseModel):
    features: List[float] = Field(..., example=[0.1, 0.5, 0.2, 0.8, 1.5])
    feature_names: Optional[List[str]] = None

# Output for the prediction endpoint
class PredictionOutput(BaseModel):
    prediction_label: str = Field(..., example="N2")
    prediction_id: int = Field(..., example=3)
    confidence_score: Optional[float] = Field(None, example=0.85)
