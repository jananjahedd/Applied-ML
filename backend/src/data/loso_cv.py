...
from src.utils.paths import get_processed_data_dir, get_repo_root, get_splits_data
...
def load_preprocessed_data() -> Tuple[List[mne.Epochs], List[str]]:
    ...
    if not processed_files:
        logger.error(
            f"No epoch files (*-epo.fif) found in {PROCESSED_DATA_DIR}. " "Please run the preprocessing script first."
        )
        exit()
    ...
    for filepath in processed_files:
        try:
            ...
            if len(epochs_subject) > 0:
                ...
                logger.info(f"Loaded {len(epochs_subject)} epochs for " + f"subject {subject_id_str}")
            else:
    ...
    logger.info("Successfully loaded epochs from " + f"{len(subjects_epochs_list)} subjects: {subject_ids}")
...
def prepare_data(
    ...
    logger.info(f"Combined shapes: X={X.shape}, y={y.shape}, groups={groups.shape}")
...
def splitting(
    X: np.ndarray, y: np.ndarray, groups: np.ndarray, subjects_epochs_list: List[mne.Epochs], splits_dir: pathlib.Path
) -> None:
    """Performs a subject-wise stratified split, with augmentation and \
       feature engineering.
    ...
    train_val_split = GroupShuffleSplit(n_splits=1, test_size=0.3, random_state=42)
    ...
    val_test_split = GroupShuffleSplit(n_splits=1, test_size=0.5, random_state=42)
    val_idx, test_idx = next(val_test_split.split(X_val_test, y_val_test, groups_val_test))
    ...
    for modality, ch_types_to_include in FUSION_CONFIG.items():
        logger.info(f"--- Processing all splits for {modality.upper()} modality ---")

        # select channels and create info object for the current modality
        selected_ch_indices = [idx for idx, ch_type in enumerate(all_ch_types) if ch_type in ch_types_to_include]
        ...
        if X_train_modality.shape[0] > 0:
            for i in range(X_train_modality.shape[0]):
                epoch, label, group = (X_train_modality[i], y_train[i], groups_train[i])
            ...
            logger.info(f"Augmented training set for '{modality.upper()}'" + f" shape: {X_train_aug.shape}")
        else:
            ...
            logger.info(f"Training set for {modality.upper()} is empty." + " No augmentation performed.")

        # feature Engineering
        logger.info(f"--- Applying Feature Engineering for {modality.upper()} ---")
        feature_engineer = FeatureEngineering()

        # fit on augmented training data
        logger.info(f"Fitting on augmented training data for {modality.upper()}.")
        epochs_train_aug = create_epochs_from_numpy(X_train_aug, y_train_aug, modality_info)
        X_train_featured, y_train_featured, feature_names = feature_engineer.fit(epochs_train_aug)

        # transform validation data
        logger.info(f"Transforming validation data for {modality.upper()}.")
        epochs_val = create_epochs_from_numpy(X_val_modality, y_val, modality_info)
        ...
        epochs_test = create_epochs_from_numpy(X_test_modality, y_test, modality_info)
        X_test_featured, y_test_featured = feature_engineer.transform(epochs_test)
        ...
        for split_name, (X_featured, y_featured, groups_split) in data_to_save.items():
            save_filename = f"{split_name}_{modality}_featured.npz"
            save_path = SPLITS_DIR / save_filename
            logger.info(f"Saving {split_name} featured data for '{modality}'" + f" to {save_path}")
            ...
            logger.info(f"Successfully saved featured data for '{modality.upper()}'.")

    logger.info("\n--- All data splits processed, engineered, and saved ---")


def loso_cv_main() -> None:
    """Main function to run the whole splitting logic.
