...
from sklearn.metrics import (  # type: ignore
    ...
    make_scorer, confusion_matrix, f1_score, roc_curve, auc
)
...
from src.utils.paths import get_splits_data, get_results_dir, get_repo_root
...
def load_data(
    config_key: str, splits_dir: pathlib.Path
) -> LoadedFusionData:
    """Loads and combines data for a given fusion configuration."""
    modalities_to_load = config_key.split('_')
    logger.info(
        f"Loading data for modalities: {modalities_to_load}"
        f" for config '{config_key}'"
    )
    ...
    for modality in modalities_to_load:
        ...
        if not all([train_path.exists(),
                    val_path.exists(), test_path.exists()]):
        ...
        y_train_mod, y_val_mod, y_test_mod = (
            train_data['y_train'], val_data['y_val'], test_data['y_test']
        )
        if first_y_train is None:
            first_y_train, first_y_val, first_y_test = (
                y_train_mod, y_val_mod, y_test_mod
            )
    final_X_train, final_X_val, final_X_test = (
        np.hstack(X_train_parts), np.hstack(X_val_parts),
    ...
    return (final_X_train, first_y_train, final_X_val,
            first_y_val, final_X_test, first_y_test, None, False)
...
def plot_learning_curve(
    ...
    train_sizes_abs, train_scores, val_scores = learning_curve(
        estimator, X, y, cv=stratified_kfold, n_jobs=-1,
        train_sizes=np.linspace(0.1, 1.0, 5),
        scoring=make_scorer(f1_score, average="macro", zero_division=0),
    )
    train_scores_mean, train_scores_std = (np.mean(train_scores, axis=1),
                                           np.std(train_scores, axis=1))
    val_scores_mean, val_scores_std = (np.mean(val_scores, axis=1),
                                       np.std(val_scores, axis=1))
    ...
    for class_label in all_classes:
        if class_label in present_classes:
            ...
            fpr[class_label], tpr[class_label], _ = roc_curve(
                y_true == class_label, y_proba[:, class_idx]
            )
            roc_auc[class_label] = auc(fpr[class_label], tpr[class_label])
            label = (
                f'ROC Class {class_label} (AUC = {roc_auc[class_label]:.2f})'
            )
            ax.plot(
                fpr[class_label], tpr[class_label], lw=2, label=label
            )
...
def _convert_to_json_serializable(item: Any) -> Any:
    ...
    if isinstance(item, np.ndarray):
        return item.tolist()
    if isinstance(item, (np.int_, np.intc, np.intp, np.int8, np.int16,
...
def randomforest():
    ...
    logger.info(
        "Starting Final Optimized RF experiments. "
    ...
    for config_key in CONFIGS_TO_RUN:
        ...
        X_train, y_train, X_val, y_val, X_test, y_test, _, error = load_data(
            config_key, SPLITS_DIR
        )
        ...
        random_search = RandomizedSearchCV(
            estimator=pipeline, param_distributions=param_dist, n_iter=30,
            cv=stratified_kfold, scoring=make_scorer(
        ...
        # evaluate on test
        logger.info(
            f"Evaluating best model for '{config_key}' on the TEST set"
        )
        ...
        test_report_str = classification_report(
            y_test, y_pred_test, labels=sorted_labels, zero_division=0
        )
        test_roc_auc = calculate_and_plot_roc_auc(
            best_model, X_test, y_test, sorted_labels,
        ...
        # evaluate on train
        logger.info(
            f"Evaluating best model for '{config_key}' on the TRAINING set"
        )
        ...
        train_f1_macro = f1_score(
            y_full_train, y_pred_train, average='macro', zero_division=0
        )
        train_report_str = classification_report(
            y_full_train, y_pred_train, labels=sorted_labels, zero_division=0
        )
        train_roc_auc = calculate_and_plot_roc_auc(
            best_model, X_full_train, y_full_train, sorted_labels,
        ...
        with open(metrics_path, 'w') as f:
            json.dump(
                _convert_to_json_serializable(metrics_to_save), f, indent=4
            )
        logger.info(
            f"Final metrics for '{config_key}' saved to: {metrics_path}"
        )
        ...
        plot_confusion_matrix(
            cm_test, display_labels, f"Test CM - {config_key}",
            RESULTS_DIR / f"cm_test_{config_key}.png"
        )
        cm_train = confusion_matrix(
            y_full_train, y_pred_train, labels=sorted_labels
        )
        plot_confusion_matrix(
            cm_train, display_labels, f"Train CM - {config_key}",
            RESULTS_DIR / f"cm_train_{config_key}.png"
        )

        plot_learning_curve(
            best_model, f"Final RF LC - {config_key}", X_full_train,
            y_full_train, RESULTS_DIR / f"lc_{config_key}.png"
        )
    ...
    try:
        ...
        logger.warning(
            "Pandas is not installed. To see the summary table,"
            " run: pip install pandas"
        )
        logger.info(
            f"Raw results dictionary: {json.dumps(overall_results, indent=2)}"
        )
